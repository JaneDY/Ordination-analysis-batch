...
def nmdsGroup(inType, input, infile, output, ingroup, method, pc, ps, ls, width, height):
    Rcmd = '''
library(vegan)
library(ggpubr)
\n'''
...
stress <- paste0("stress=",round(nmds$stress,4))
pc12 <- nmds$points
...
name <- paste0("'''+output+'''/nmds_by_group.pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F )
\n'''
    Rcmd += '''
write.table(nmds$points, "'''+output+'''/nmds_by_group_sites.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.nmds.group.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.nmds.group.r')
    return 'Done NMDS type: group'

def nmdsBox(inType, input, output, ingroup, method, pc, boxType, plabel, ell):
    Rcmd = '''
library(vegan)
library(ggpubr)
library(reshape2)
...
'''
        if inType == 'AF':
	        Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_'''+method+'''.pdf")
\n'''
        else:
	        Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_NMDS",pc_y,".pdf")
\n'''
        Rcmd += '''
...
'''
        if inType == 'AF':
            Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_NMDS",pc_y,"_'''+method+'''.pdf")
\n'''
        else:
            Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_NMDS",pc_x,".pdf")
\n'''
        Rcmd += '''
...
'''
        if inType == 'AF':
	        Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_NMDS",pc_x,"_NMDS_",pc_y,"_'''+method+'''.pdf")
\n'''
        else:
	        Rcmd += '''
name <- paste0("'''+output+'''/nmds_with_boxplot_NMDS",pc_x,"_NMDS",pc_y,".pdf")
\n'''
        Rcmd += '''
...
\n'''
    Rcmd += '''
write.table(nmds$points, "'''+output+'''/nmds_with_boxplot_sites.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.nmds.box.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.nmds.box.r')
    return 'Done NMDS type: box'

def nmdsDiv(inType, input, infile, output, ingroup, adiv, index, method, plabel, width, height):
    Rcmd = '''
library(vegan)
library(ggpubr)
library(ggsci)
...
mytheme <- theme(panel.border = element_rect(color = "black",size = 1.0,fill = NA),
                 ...
                 plot.margin = unit(c(2.3, 5, 8, 2), "lines"))
...
p <- p + annotate("text", x=max(pc12$pc_x)*0.9, y=min(pc12$pc_y), 
                  label=stress, size=4.5, family="serif", fontface="italic")
\n'''
    if plabel == 'T':
        Rcmd += '''
p <- p + geom_text(aes(label = pc12$sample), size=3, colour = '#595959', vjust=1.5, hjust=-0.2)
...
name <- paste0("'''+output+'''/nmds_with_'''+index+'''_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(nmds$points, "'''+output+'''/nmds_with_adiv_sites.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.nmds.div.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.nmds.div.r')
    return 'Done NMDS type: div'

def nmdsAbu(inType, input, infile, output, ingroup, abu, taxList, topTaxon, method, plabel, width, height):
    Rcmd = '''
library(vegan)
library(ggpubr)
library(ggsci)
...
p <- p + annotate("text", x=max(pc12$pc_x)*0.85, y=min(pc12$pc_y), label=stress, size=5, family="serif", fontface="italic")
\n'''
    if plabel == 'T':
        Rcmd += '''
p <- p + geom_text(aes(label = pc12$sample), size=3, colour = '#595959', vjust=1.5, hjust=-0.2)
...
name <- paste0("'''+output+'''/nmds_with_abund_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(nmds$points, "'''+output+'''/nmds_with_abund_sites.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.nmds.abu.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.nmds.abu.r')
    return 'Done NMDS type: abu'

def nmdsRichness(inType, input, output, ingroup, method, otu, pc, plabel, width, height):
    Rcmd = '''
library(vegan)
library(ggpubr)
library(ggsci)
...
name <- paste0("'''+output+'''/nmds_with_richness_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(nmds$points, "'''+output+'''/nmds_with_richness_sites.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.nmds.richness.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.nmds.richness.r')
    return 'Done NMDS type: richness'

def paras():
    parser = argparse.ArgumentParser(description='Do Non-metric Multidimensional Scaling Analysis.', epilog='What can I help U? Please contact yun.ding@majorbio.com.')
    parser.add_argument('-type', required = True, choices = ['group','box','div','abu','richness'], help = 'Choose which type of nmds you want to plot')
...
if __name__ == '__main__':
	...
	if not ingroup:
	    ...
	    nmdsGroup(inType, input, infile, output, ingroup, method, pc, ps, ls, width, height)
    if type == 'box':
	if not ingroup:
	    exit('Please input group file')
	else:
	    nmdsBox(inType, input, output, ingroup, method, pc, boxType, plabel, ell)
    if type == 'div':
	if not args.div or not args.index:
	    ...
	    index = args.index
	    nmdsDiv(inType, input, infile, output, ingroup, adiv, index, method, plabel, width, height)
    if type == 'abu':
	if not args.ab:
	...
	topTaxon = args.top
	nmdsAbu(inType, input, infile, output, ingroup, abu, taxList, topTaxon, method, plabel, width, height)
    if type == 'richness':
	if not args.otu:
	    exit('Please input otu table by -otu')
	else:
	    otu = os.path.abspath(args.otu)
	    nmdsRichness(inType, input, output, ingroup, method, otu, pc, plabel, width, height)
