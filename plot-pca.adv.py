...
def pcaGroup(input, output, ingroup, pc, ps, ls, width, height):
    Rcmd = '''
library(ggpubr)

da <- read.table("'''+input+'''",sep="\\t",head=T,comment.char = "",check.names = FALSE)
...
sd_x <- tapply(pc12$pc_x, pc12$group, sd)/sqrt(table(pc12$group)) # SEM: sd/square-root(N)
...
name <- paste0("'''+output+'''/pca_by_group_PC",pc_x,"_PC",pc_y,".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)

write.table(pca$x, "'''+output+'''/pca_by_group_sites.xls", sep="\\t", col.names=NA)
write.table(summary(pca)$importance[2,], "'''+output+'''/pca_by_group_importance.xls", sep="\\t")
write.table(pca$rotation, "'''+output+'''/pca_by_group_rotation.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.pca.group.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.pca.group.r')
    return 'Done PCA type: group'

def pcaBox(input, output, ingroup, pc, boxType, plabel, ell):
    Rcmd = '''
library(ggpubr)
...
name <- paste0("'''+output+'''/pca_with_boxplot_PC",pc_y,".pdf")
pdf(name, width = 11, height = 8)
ggarrange(p, pr, ncol = 2, nrow = 1, widths = c(2,1), heights = c(1,1))#, align = "h")
dev.off()
\n'''
    Rcmd += '''
pt <- ggboxplot(pc12, x="group", y="pc_x", fill = "group", add = "jitter", palette = "npg") + 
   coord_flip() + 
   stat_compare_means(comparisons = comp, label = "p.signif")
...
name <- paste0("'''+output+'''/pca_with_boxplot_PC",pc_x,".pdf")
...
name <- paste0("'''+output+'''/pca_with_boxplot_PC",pc_x,"_PC",pc_y,".pdf")
pdf(name, width = 10, height = 10)
ggarrange(pt, p0, p, pr, ncol = 2, nrow = 2, widths = c(2,1), heights = c(1,2))#, align = "hv")
dev.off()
\n'''
    Rcmd += '''
write.table(pca$x, "'''+output+'''/pca_with_boxplot_sites.xls", sep="\\t", col.names=NA)
write.table(summary(pca)$importance[2,], "'''+output+'''/pca_with_boxplot_importance.xls", sep="\\t")
write.table(pca$rotation, "'''+output+'''/pca_with_boxplot_rotation.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.pca.box.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.pca.box.r')
    return 'Done PCA type: box'

def pcaDiv(input, output, ingroup, adiv, index, pc, plabel, width, height):
    Rcmd = '''
library(ggpubr)
library(ggsci)
...
mytheme <- theme(panel.border = element_rect(color = "black",size = 1.0,fill = NA),
                 ...
                 plot.margin = unit(c(2.3, 4, 8, 2), "lines"))
...
\n'''
    if ingroup:
        Rcmd += '''
p <- ggplot(pc12, aes(x=pc_x, y=pc_y, fill=index, color=index, shape=group)) + geom_point(size=4, stroke=1) + 
...
name <- paste0("'''+output+'''/pca_with_'''+index+'''_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+args.ww+''', height = '''+args.hh+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(pca$x, "'''+output+'''/pca_with_adiv_sites.xls", sep="\\t", col.names=NA)
write.table(summary(pca)$importance[2,], "'''+output+'''/pca_with_adiv_importance.xls", sep="\\t")
write.table(pca$rotation, "'''+output+'''/pca_with_adiv_rotation.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.pca.div.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.pca.div.r')
    return 'Done PCA type: div'

def pcaAbu(input, output, ingroup, abu, taxList, topTaxon, plabel, pc, width, height):
    Rcmd = '''
library(ggpubr)
library(ggsci)

data <- read.table("'''+input+'''",sep="\\t",head=T,check.names = FALSE)
...
name <- paste0("'''+output+'''/pca_with_abund_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+args.ww+''', height = '''+args.hh+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(pca$x, "'''+output+'''/pca_with_abund_sites.xls", sep="\\t", col.names=NA)
write.table(summary(pca)$importance[2,], "'''+output+'''/pca_with_abund_importance.xls", sep="\\t")
write.table(pca$rotation, "'''+output+'''/pca_with_abund_rotation.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.pca.abu.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.pca.abu.r')
    return 'Done PCA type: abu'

def pcaRichness(input, output, ingroup, pc, plabel, width, height):
    Rcmd = '''
library(ggpubr)
library(ggsci)
...
name <- paste0("'''+output+'''/pca_with_richness_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(pca$x, "'''+output+'''/pca_with_richness_sites.xls", sep="\\t", col.names=NA)
write.table(summary(pca)$importance[2,], "'''+output+'''/pca_with_richness_importance.xls", sep="\\t")
write.table(pca$rotation, "'''+output+'''/pca_with_richness_rotation.xls", sep="\\t", col.names=NA)
\n'''
    open(output+'/cmd.pca.richness.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.pca.richness.r')
    return 'Done PCA type: richness'

def paras():
    parser = argparse.ArgumentParser(description='Do Principal Component Analysis.', epilog='What can I help U? Please contact yun.ding@majorbio.com.')
    parser.add_argument('-type', required = True, choices = ['group','box','div','abu','richness'], help = 'Choose which type of pca you want to plot')
    parser.add_argument('-i', required = True, help = 'Input abundance file')
    ...
    parser.add_argument('-ps', default = '15', help = 'Point size for type group, default: 15')
    parser.add_argument('-ls', default = '6', help = 'Label size for type group, default: 6')
...
if __name__ == '__main__':
	...
	if not ingroup:
	    ...
	    pcaGroup(input, output, ingroup, pc, ps, ls, width, height)
    if type == 'box':
        if not ingroup:
    	    exit('Please input group file')
        else:
            pcaBox(input, output, ingroup, pc, boxType, plabel, ell)
    if type == 'div':
        if not args.div or not args.index:
    	    ...
    	    index = args.index
    	    pcaDiv(input, output, ingroup, adiv, index, pc, plabel, width, height)
    if type == 'abu':
        if not args.ab:
    	    ...
    	    topTaxon = args.top
    	    pcaAbu(input, output, ingroup, abu, taxList, topTaxon, plabel, pc, width, height)	
    if type == 'richness':
        pcaRichness(input, output, ingroup, pc, plabel, width, height)
