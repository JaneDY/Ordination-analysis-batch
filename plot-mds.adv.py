...
def mdsGroup(inType, input, infile, output, ingroup, method, pc, ps, ls, width, height):
    Rcmd = '''
library(vegan)
library(bios2mds) #mmds
library(ggpubr)
\n'''
    if inType == 'AF':
        Rcmd += '''
...
mds <- mmds(as.matrix(dist))
mds_importance <- mds$eigen.perc
mdsNum <- as.numeric(unlist(strsplit("1-2","-")))
mdsX <- mdsNum[1]
mdsY <- mdsNum[2]
pc12 <- mds$coord[,mdsNum]
...
name <- paste0("'''+output+'''/mds_by_group.pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F )
\n'''
    Rcmd += '''
write.table(mds$coord, "'''+output+'''/mds_by_group_sites.xls", sep="\\t", col.names=NA)
write.table(mds$eigen.perc, "'''+output+'''/mds_by_group_importance.xls", sep="\\t")
\n'''
    open(output+'/cmd.mds.group.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.mds.group.r')
    return 'Done MDS type: group'

def mdsBox(inType, input, output, ingroup, method, pc, boxType, plabel, ell):
    Rcmd = '''
library(vegan)
library(bios2mds) #mmds
library(ggpubr)
...
p <- ggscatter(pc12, x = "pc_x", y = "pc_y", 
               color = "group", shape = "group", size = 3, palette = "npg")
...
\n'''
    Rcmd += '''
pr <- ggboxplot(pc12, x="group", y="pc_y", fill = "group", add = "jitter", palette = "npg") +   stat_compare_means(comparisons = comp, label = "p.signif")
...
'''
        if inType == 'AF':
	        Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_'''+method+'''.pdf")
\n'''
        else:
	        Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_MDS",pc_y,".pdf")
\n'''
        Rcmd += '''
...
\n'''
    Rcmd += '''
pt <- ggboxplot(pc12, x="group", y="pc_x", fill = "group", add = "jitter", palette = "npg") +   coord_flip() +  stat_compare_means(comparisons = comp, label = "p.signif")
...
'''
        if inType == 'AF':
            Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_MDS",pc_y,"_'''+method+'''.pdf")
\n'''
        else:
            Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_MDS",pc_x,".pdf")
\n'''
        Rcmd += '''
...
'''
        if inType == 'AF':
	        Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_MDS",pc_x,"_MDS_",pc_y,"_'''+method+'''.pdf")
\n'''
        else:
	        Rcmd += '''
name <- paste0("'''+output+'''/mds_with_boxplot_MDS",pc_x,"_MDS",pc_y,".pdf")
\n'''
        Rcmd += '''
...
\n'''
    Rcmd += '''
write.table(mds$coord, "'''+output+'''/mds_with_boxplot_sites.xls", sep="\\t", col.names=NA)
write.table(mds$eigen.perc, "'''+output+'''/mds_with_boxplot_importance.xls", sep="\\t")
\n'''
    open(output+'/cmd.mds.box.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.mds.box.r')
    return 'Done MDS type: box'

def mdsDiv(inType, input, infile, output, ingroup, adiv, index, method, plabel, width, height):
    Rcmd = '''
library(vegan)
library(bios2mds) #mmds
library(ggpubr)
...
mytheme <- theme(panel.border = element_rect(color = "black",size = 1.0,fill = NA),
                 ...
                 plot.margin = unit(c(2.5, 5, 8, 2), "lines"))

mypch <-c(21:25,7:14)

p <- ggplot(pc12, aes(x=pc_x, y=pc_y, fill=index)) + geom_point(shape=21, size=4, stroke=0.1) + 
  scale_color_gsea() + 
  scale_fill_gsea(guide=guide_colorbar(barwidth = 13, direction = "horizontal", 
  title = "'''+index+'''", title.position = "top"))
...
name <- paste0("'''+output+'''/mds_with_'''+index+'''_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(mds$coord, "'''+output+'''/mds_with_adiv_sites.xls", sep="\\t", col.names=NA)
write.table(mds$eigen.perc, "'''+output+'''/mds_with_adiv_importance.xls", sep="\\t")
\n'''
    open(output+'/cmd.mds.div.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.mds.div.r')
    return 'Done MDS type: div'

def mdsAbu(inType, input, infile, output, ingroup, abu, taxList, topTaxon, method, plabel, width, height):
    Rcmd = '''
library(vegan)
library(bios2mds) #mmds
library(ggpubr)
...
name <- paste0("'''+output+'''/mds_with_abund_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(mds$coord, "'''+output+'''/mds_with_abund_sites.xls", sep="\\t", col.names=NA)
write.table(mds$eigen.perc, "'''+output+'''/mds_with_abund_importance.xls", sep="\\t")
\n'''
    open(output+'/cmd.mds.abu.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.mds.abu.r')
    return 'Done MDS type: abu'

def mdsRichness(inType, input, output, ingroup, method, otu, pc, plabel, width, height):
    Rcmd = '''
library(vegan)
library(bios2mds) #mmds
library(ggpubr)
...
name <- paste0("'''+output+'''/mds_with_richness_PC", pc_x, "-PC", pc_y, ".pdf")
ggsave(name, p, width = '''+width+''', height = '''+height+''', limitsize = F)
\n'''
    Rcmd += '''
write.table(mds$coord, "'''+output+'''/mds_with_richness_sites.xls", sep="\\t", col.names=NA)
write.table(mds$eigen.perc, "'''+output+'''/mds_with_richness_importance.xls", sep="\\t")
\n'''
    open(output+'/cmd.mds.richness.r', 'w').write(Rcmd)
    os.system('Rscript '+output+'/cmd.mds.richness.r')
    return 'Done MDS type: richness'

def paras():
    parser = argparse.ArgumentParser(description='Do metric Multidimensional Scaling Analysis.', epilog='What can I help U? Please contact yun.ding@majorbio.com.')
    parser.add_argument('-type', required = True, choices = ['group','box','div','abu','richness'], help = 'Choose which type of mds you want to plot')
    ...
    parser.add_argument('-div', help = 'Input alpha diversity index file, format: sampleID	index1	index2	...')
    parser.add_argument('-index', help = 'Choose which index column to use')

    parser.add_argument('-ab', help = 'Input taxon abundance file')
    group = parser.add_mutually_exclusive_group()
    group.add_argument('-l', '--list', default = '', help = 'Taxon list, whose abundance will be used to set point size')
    group.add_argument('-t', '--top', default = '50', help = 'Top abundance taxon to set point size, default: 50')
    ...
    parser.add_argument('-m', choices = ["manhattan", "euclidean", "canberra", "clark", "bray", "kulczynski", "jaccard", "gower", "altGower", "morisita", "horn", "mountford", "raup", "binomial", "chao", "cao" or "mahalanobis"], default= 'bray', help = 'When input an abundance file, then choose one dissimilarity index to generate the distance matrix')
    parser.add_argument('-ww', default = '10', help = 'Set figure width, default: 10')
    parser.add_argument('-hh', default = '10', help = 'Set figure height, default: 10')
...
if __name__ == '__main__':
	...
	if not ingroup:
	    ...
	    mdsGroup(inType, input, infile, output, ingroup, method, pc, ps, ls, width, height)
    if type == 'box':
	if not ingroup:
	    exit('Please input group file')
	else:
	    mdsBox(inType, input, output, ingroup, method, pc, boxType, plabel, ell)
    if type == 'div':
	if not args.div or not args.index:
	    exit('Please input diversity index file and choose one index')
	else:
	    adiv = os.path.abspath(args.div)
	    index = args.index
	    mdsDiv(inType, input, infile, output, ingroup, adiv, index, method, plabel, width, height)
    if type == 'abu':
	if not args.ab:
	    exit('Please input taxon abundance file')
	abu = os.path.abspath(args.ab)
	taxList = os.path.abspath(args.list)
	topTaxon = args.top
	mdsAbu(inType, input, infile, output, ingroup, abu, taxList, topTaxon, method, plabel, width, height)
    if type == 'richness':
	if not args.otu:
	    exit('Please input otu table by -otu')
	else:
	    otu = os.path.abspath(args.otu)
	    mdsRichness(inType, input, output, ingroup, method, otu, pc, plabel, width, height)
